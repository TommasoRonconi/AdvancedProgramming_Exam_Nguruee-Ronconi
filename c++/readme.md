...
To compile the application type `make` from the root directory, it will generate an executable `tree.x` that takes no command line arguments.
It will fill a Tree with a set of values, then balance it and print it on stdout.

Three pre-processor variables are provided and can be included by means of the command `make`.
By typing
```bash
make OPT=-DTEST
```
...
Last possible pre-processor option is compiled by typing
```bash
make OPT=-DTEST_BALANCE
```
It takes same arguments as the `-DPERFORMANCE` version (couples key-value from stdin) and measures the time spent by the `balance()` function.
